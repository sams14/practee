...
const userRegister = async (userDets, role, res) => {
  try {
    ...
    if (!emailNotRegistered) {
      return res.render("pages/create-account", {
        response: {
...
const userLogin = (role) => async (req, res, next) => {
  await passport.authenticate("local", async (err, user, info) => {
    ...
    if (!user) {
      message =
        "This Email-Id Hasn't Been Registered Yet !! Please Create An Account";
      return res.redirect(
        303,
        "/utility/login-" + role + "/?success=" + false + "&message=" + message
      );
    }
    if (role != user.role) {
      message = "Please Make Sure You Are Logging In From The Right Portal";
...
const checkLogin = (req, res, next) => {
  if (req.isAuthenticated()) {
    return res.redirect(303, "/utility/profile");
  } else {
    next();
  }
};
