	...
	while True:
		...
		if (json_response['total'] > 0):
			for record in json_response['data']:
				folders.append({
					"id": record['uri'][record['uri'].rindex('/')+1:len(record['uri'])],
					"name" : record['name'],
...
def filter_oldfolders(folders) :
	...
	for folder_details in folders:
		...
		folder_modified = datetime.datetime.fromisoformat(folder_details["modified_time"])
		if (date_now-folder_modified).days >= 250:
...
def delete_folder_videos(folder_details):
	...
	while True:
		query = {'per_page':50, 'page':counter}
		videos_url = 'https://api.vimeo.com/users/{user_id}/projects/{project_id}/videos'.format(user_id= vimeo_userid, project_id=folder_details["id"])
		response = requests.get(videos_url, headers=headers)
		json_response = json.loads(response.text)

		if (json_response['total'] > 0):
			print('Fetching Videos In {folders}'.format(folders=folder_details["name"]))
			for record in json_response['data']:
	...
	print('\n'+'Deleting videos from folders {folders}'.format(folders=folder_details["name"]))
	headers = headers = {'authorization': 'Bearer '+ vimeo_token}
	url = 'https://api.vimeo.com/videos/'
	for video_id in videos:
		response = requests.delete(url + video_id, headers=headers)
		if response.status_code != 200 and response.status_code != 204:
			print('Error Status: {status} \nFailed To Delete Video Id:  {video_id} !!'.format(video_id=video_id, status=response.status_code))
...
def delete_vimeo_folders(records):
	...
	for folder_details in records:
		...
		delete_folder_videos(folder_details)
		response = requests.delete(url + folder_details["id"], headers=headers, params=query)

		if response.status_code == 200 or response.status_code == 204:
			print('Deleted Folder {folders} Successfully !!'.format(folders=folder_details["name"]))
		else:
			print('Error Status: {status} \nFailed To Deleted Folder {folders} !!'.format(folders=folder_details["name"], status=response.status_code))
	return
