...
class Utils:
    ...
    def get_zoom_users(self):
        ...
        query = {"page_size": "50", "status": "active"}
    ...
    def get_zoom_files(self, users, start_date, end_date):
        ...
        for user in users["users"]:
            ...
            while to_date < end_date + timedelta(days=1):
                        ...
                        if meeting["recording_count"] > 0:
                            for recording in meeting["recording_files"]:
                                try:
                                    ...
                                    item["recording_start"] = recording[
                                        "recording_start"
                                    ]
                                    item["recording_end"] = recording["recording_end"]
                                    item["download_url"] = recording[
                                        "download_url"
                                    ]  # urllib.parse.quote(recording['download_url'])
                                    try:
                                    ...
                                    filename = (
                                        datetime.strptime(
                                            item["recording_start"],
                                            "%Y-%m-%dT%H:%M:%SZ",
                                        ).strftime("GMT%Y%m%d-%H%M%S")
                                        + str(index)
                                        + "."
                                        + str(item["file_extension"])
                                    )
                                    item["file_name"] = str(filename)
                                    filepath = "./meetings/{username}/{topic}/".format(
                                        username=item["username"], topic=item["topic"]
                                    )
                ...
                if (to_date > end_date) and (
                    (to_date - timedelta(days=30)) != end_date
                ):
                    to_date = end_date
    ...
    def get_records(self, argv, scriptname):
        ...
        if len(argv) < 2:
            print(
                "You must use: python {scriptname} --inputfile inputfile.csv --ouputfile outputfile.csv or python {scriptname} --rangedates yyyy-mm-dd yyyy-mm-dd --ouputfile ouputfile.csv".format(
        ...
        if argv[1] == params[0]:
            if len(argv) < 5:
                print(
                    "You must use: python {scriptname} --inputfile inputfile.csv --ouputfile outputfile.csv".format(
                        scriptname=scriptname
                    )
                )
                exit()
            ...
            if len(argv) < 6:
                print(
                    "You must use: python {scriptname} --rangedates yyyy-mm-dd yyyy-mm-dd --ouputfile ouputfile.csv".format(
                        scriptname=scriptname
                    )
                )
                exit()
                ...
                self.files = self.get_zoom_files(
