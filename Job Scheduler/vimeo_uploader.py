...
def check_valid_meeting(meetingId, date=None):
    # print(" Verifying Meeting Validity ".center(100, ":"))

    url = "https://api.zoom.us/v2/metrics/meetings/" + meetingId
    query = {"from": date, "to": date, "type": "past"}
...
def create_vimeo_folder(foldername):
    print(
        "\n"
        + " Create Vimeo Folder {foldername} ".format(foldername=foldername).center(
            100, ":"
        )
    )
    headers = headers = {"authorization": "Bearer " + utils.vimeo_token}
    url = "https://api.vimeo.com/users/{user_id}/projects".format(
        user_id=utils.vimeo_userid
    )
    body = {}
    ...
    if response.status_code == 201:
        ...
        folder[foldername] = json_response["uri"][
            json_response["uri"].rindex("/") + 1 : len(json_response["uri"])
        ]
    ...
    while True:
        ...
        if json_response["total"] > 0:
            for record in json_response["data"]:
                folders[record["name"]] = record["uri"][
...
def request_move_videos_to_folder(videos_list, record, folder_id):
    ...
    if response.status_code != 204:
        print(
            "Error trying to move videos videos {videos} to folder {folder}".format(
                videos=videos_str, folder=record
            )
        )
    else:
        print(
            "Moved videos {videos} to folder {folder}".format(
                videos=videos_list[record], folder=record
            )
        )
...
def set_embeded_presets(record):
    ...
    headers = headers = {"authorization": "Bearer " + utils.vimeo_token}
    url = "https://api.vimeo.com/videos/{video_id}/presets/{preset_id}".format(
        video_id=record["vimeo_id"], preset_id=utils.vimeo_preset_id
    )
    ...
    with open(os.path.abspath("Job Scheduler/" + filename), mode="w") as f:
        ...
        for record in records:
            if record["file_extension"] == "MP4":
                transcript_record = Transcript().find_transcript_record(
                    records, record["meeting_uuid"]
                )
                if transcript_record or check_valid_meeting(
                    record["meeting_uuid"], record["recording_start"].split("T")[0]
                ):
                    if (
                        record["vimeo_status"] != "available"
                        and record["vimeo_status"] != "error"
                        and record["vimeo_uri"] != ""
                    ):
                        ...
                        if (
                            ...
                            if record["vimeo_status"] == "available":
                                ...
                                print(
                                    "Transcoding video %s... almost ready"
                                    % record["file_name"]
                                )
    ...
    for record in records:
        if record["file_extension"] == "MP4":
            ...
            if transcript_record or check_valid_meeting(
                ...
                if (
                    record["vimeo_status"] != "available"
                    and record["vimeo_status"] != "transcoding"
                    and record["vimeo_status"] != "transcode_starting"
                ):
                    print(
                        "\n"
                    ...
                    body["name"] = (
                        record["topic"]
                        + record["recording_start"].split("T")[0]
                        + "."
                        + record["file_extension"]
                    )
                    body["description"] = VIDEO_DESCRIPTION.format(
                        topic=record["topic"], start_date=record["recording_start"]
                    )
                    ...
                    upload["link"] = (
                        record["download_url"] + "?access_token=" + utils.zoom_token
                    )
                    ...
                    if response.status_code == 201:
                        ...
                        record["vimeo_transcode_status"] = json_response["transcode"][
                            "status"
                        ]
                        record["vimeo_id"] = record["vimeo_uri"][
                            8 : len(record["vimeo_uri"])
                        ]
                else:
                    print(
                        "\n"
                        + "Record {filename} already or almost uploaded! ".format(
                            filename=record["file_name"]
                        )
                    )
    return records
...
if __name__ == "__main__":
    ...
    with open(os.path.abspath("Job Scheduler/error.txt"), "w") as f:
        f.write(
            (" Zoom Vimeo Integration Report : " + date + " !! ").center(100, ":")
            + "\n"
        )
